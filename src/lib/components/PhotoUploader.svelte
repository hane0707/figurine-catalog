  ...
  async function uploadWithRetry(url: string, blob: Blob, maxRetries = 3): Promise<void> {
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        // 403, 413 などはリトライしても無意味なので即座に throw
        if (res.status === 403 || res.status === 413 || (res.status >= 400 && res.status < 500)) {
          throw err;
        }
        throw err;
      } catch (e) {
        if (i === maxRetries - 1) throw e;
        // 4xx は即座に再throw（リトライ不要）
        if ((e as any)?.status >= 400 && (e as any)?.status < 500) throw e;
